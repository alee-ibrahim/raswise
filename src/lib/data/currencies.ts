...
export const CURRENCIES: Currency[] = [
  ...
  { code: "UZS", name: "Uzbekistan Som", symbol: "сум" },
...
export function getCurrency(code: string | undefined | null): Currency | null {
  if (!code) return null;
  return BY_CODE.get(code.toUpperCase()) || null;
}

export function formatCurrency(amount: number, code: string | undefined | null): string {
  const c = getCurrency(code);
  const fixed = amount.toFixed(2);
  return c ? `${fixed} ${c.code}` : fixed;
}
